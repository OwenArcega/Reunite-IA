document.addEventListener("DOMContentLoaded", function () {
    ...
    const mascotaadoptada = mascotasadoptadas.find((m) => m.id === parseInt(id));
            ...
            if (mascotaadoptada) {
              ...
              for (const propiedad in mascotaadoptada) {
                if (mascotaadoptada.hasOwnProperty(propiedad)) {
                  const valor = mascotaadoptada[propiedad];
                  ...
                  if(propiedad.toLowerCase() === 'contacto'){
                    ...
                    for(const info in valor){
                      ...
                      input_info.value = mascotaadoptada['contacto'][info]
  ...
  fetch('https://nominatim.openstreetmap.org/search?format=json&q=' + mascotaadoptada.ubicacion)
  ...
  function eliminarMascota(id) {
    const index = mascotasadoptadas.findIndex(mascotaadoptada => {
      if(mascotaadoptada.id == id){
...
btn_guardar_cambios.addEventListener('click',()=>{
  const index = mascotasadoptadas.findIndex(mascotaadoptada => {
    if(Number(mascotaadoptada.id) == Number(id)){
      return mascotaadoptada
    }
  });
